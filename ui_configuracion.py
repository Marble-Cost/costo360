# ui_configuracion.py
# ─────────────────────────────────────────────────────────────────────────────
# Módulo de Configuración: Perfil de Empresa, Finanzas, Logo y Usuarios.
# Extraído de app.py (Strangler Fig — Fase 6).
#
# REGLA ARQUITECTÓNICA: este archivo NUNCA importa app.py.
# Todas las dependencias de BD se inyectan como parámetros.
#
# Firma:
#   _ui_configuracion(
#       fn_guardar_config,   # (clave: str, valor) -> None
#       fn_guardar_logo,     # (logo_bytes: bytes) -> None
#       fn_crear_usuario,    # (username, password, pin, rol, nombre) -> bool
#       fn_listar_usuarios,  # () -> list[tuple]
...
def _ui_configuracion(
    ...
    """Renderiza la pestaña completa de Configuración."""
    ...
    if tab_usuarios is not None:
        with tab_usuarios:
            ...
            # Validación y ejecución fuera del form para poder mostrar mensajes
            if _submit_form:
                _err_form = []
            ...
            if not _todos_usr:
                ...
                for _hcol, _hlbl in zip(
                    [_hc0, _hc1, _hc2, _hc3], ["#", "Nombre / Username", "Rol", "Acción"]
                ):
                    _hcol.markdown(
