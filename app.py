# app.py — CostoMármol v5 · Adaptive UX Edition
...
def seccion_titulo(texto, subtexto=""):
    sub = f'<div style="opacity:0.7;font-size:0.82rem;font-weight:400;margin-top:3px">{subtexto}</div>' if subtexto else ""
...
def linea_costo(label, valor, destacado=False, cero_gris=True):
    ...
    borde = f"border-top:2px solid var(--text-color);padding-top:10px;margin-top:6px;" if destacado else ""
...
with st.sidebar:
    ...
    if st.session_state.get("logo_bytes"):
        try: st.image(st.session_state.logo_bytes, use_container_width=True); _logo_shown = True
        except: pass
    if not _logo_shown:
        st.markdown(
    ...
    st.markdown(f'<div style="margin:10px 0 16px;padding:0 2px"><div style="font-size:0.82rem;font-weight:700;line-height:1.3">{st.session_state.empresa_info["nombre"]}</div></div>', unsafe_allow_html=True)
    
    opciones_menu = ["Inicio", "Cotizacion Directa", "Cotizacion AIU", "Historial", "Dashboard", "Parametros", "Asistente IA", "Configuracion"]
    
    # Callback to update radio safely
    def update_nav():
    ...
    st.markdown(
        f"""<div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);
          ...
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
            ...
            <div>
              <div style="font-size:0.72rem;opacity:0.6;text-transform:uppercase;letter-spacing:0.08em">Paso {_paso_idx + 1} de {_total}</div>
...
if pagina == "Inicio":
    ...
    <div style="background:linear-gradient(135deg,var(--primary-color) 0%,var(--secondary-background-color) 200%);
      ...
      <div style="opacity:0.7;font-size:0.92rem;line-height:1.65;max-width:500px">
        ...
        Cotización, logística, AIU y PDF adaptables a tu entorno.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("---")
    if st.button("🚀 Reactivar Guía Instructiva (Tour)", use_container_width=True):
        st.session_state.tour_activo = True
        st.session_state.tour_paso = 0
        st.rerun()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if pre:
        alerta("Datos cargados exitosamente (desde IA o Historial). Revisa y ajusta lo que necesites.", "bueno")
    ...
    with c1:
        ...
        # Try to find pre_ref in refs_cat
    ...
    with col_veh:
        ...
        veh_lbl = st.selectbox("Vehiculo de entrega", _veh_keys)
    ...
    if st.button("Calcular cotizacion", type="primary", use_container_width=True):
        ...
        # Save complete state so it can be reloaded later!
    ...
    if st.session_state.cotizacion and st.session_state.cotizacion.get("tipo_proyecto") != "Licitación AIU":
        ...
        with col_res:
            bloque_costos([
                ("Material", r['c1_material']), ("Produccion", r['c2_mano_obra']),
                ("Zocalos", r['c3_zocalos']), ("Insumos", r['c4_insumos']),
                ("Logistica", r['c5_logistica']), ("Viaticos", r['c6_viaticos']),
        ...
        with col_det:
            ...
            c2a.metric("Costo/m² instalado", numero_completo(r['costo_total']/max(r['m2_real'],1)))
            st.markdown(f"<div style='font-weight:700;margin:14px 0 8px'>Analisis real</div>", unsafe_allow_html=True)
            precio_real = st.number_input("Precio a cobrar realmente (opcional)", min_value=0, value=0, step=10_000)
            if precio_real > 0:
                analisis = analizar_precio_real(precio_real, r['costo_total'], r['precio_sugerido'])
                alerta(f"Margen real: <strong>{analisis['margen_real']:.1f}%</strong> — Utilidad: {numero_completo(analisis['utilidad_real'])}", analisis["estado"])
        ...
        with colp1:
            num_cot = st.text_input("Numero", value=f"COT-{datetime.today().strftime('%Y')}-001", key="num_cot")
            if st.button("Descargar Cotizacion PDF", type="primary", use_container_width=True):
                ...
                st.download_button("Guardar PDF", pdf_bytes, file_name=f"{num_cot}_Cotizacion.pdf", mime="application/pdf", use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    nombre_cliente_aiu = st.text_input("Nombre del cliente o proyecto", placeholder="Ej: Constructora ABC", value=st.session_state.pre.get("nombre_cliente", ""))
    ...
    st.markdown(f"<div style='font-weight:700;margin:10px 0'>Costo Directo: {numero_completo(cd_total)}</div>", unsafe_allow_html=True)
    ...
    with c4: veh_aiu_lbl = st.selectbox("Vehiculo", list(VEHICULOS.keys()))
    
    vehiculo_aiu = VEHICULOS[veh_aiu_lbl]
    col1, col2 = st.columns(2)
    km_aiu = col1.number_input("Km", value=10.0)
    peajes_aiu = col2.number_input("Peajes", value=0)

    if st.button("Calcular y Guardar AIU", type="primary", use_container_width=True):
        res_aiu = calcular_aiu(cd_total, pct_a, pct_i, pct_u, vehiculo_aiu, km_aiu, peajes_aiu, False, False, "pueblo", 0, 2)
        # Adapt for DB saving
        res_aiu["tipo_proyecto"] = "Licitación AIU"
        res_aiu["categoria"] = "Múltiple"
        res_aiu["m2_real"] = 0
        ...
        _num_auto = f"AIU-{date.today().strftime('%Y%m%d')}-01"
        _guardar_cotizacion(_num_auto, nombre_cliente_aiu, res_aiu)
        st.success("Cotización AIU guardada en el historial.")

    if st.session_state.cotizacion and st.session_state.cotizacion.get("tipo_proyecto") == "Licitación AIU":
        r = st.session_state.cotizacion
        hero_banner("Precio total del contrato", numero_completo(r['precio_total']), f"CD: {numero_completo(r['cd'])}")
        bloque_costos([
            ("Costo Directo (CD)", r['cd']),
            (f"A — Administracion ({pct_a}%)", r['val_a']),
            (f"I — Imprevistos ({pct_i}%)", r['val_i']),
            (f"U — Utilidad ({pct_u}%)", r['val_u']),
            ("IVA 19% sobre Utilidad", r['val_iva']),
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not _rows:
        ...
        for _col, _lbl in zip(_hdr, ["Numero", "Fecha", "Cliente", "Material", "Precio", "Estado", "Acción"]):
        ...
        for _row in _rows:
            ...
            if _cols[6].button("✏️", key=f"ed_{_rid}", help="Cargar y editar esta cotización"):
                try:
                    datos = json.loads(_rjson)
                    estado_guardado = datos.get("_estado_guardado", {})
                    ...
                    st.error("No se pudo cargar el formato antiguo.")
