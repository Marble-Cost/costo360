...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.get("modo_express"):
        ...
        # ── Mapa de tipo de proyecto inferido desde el ancho elegido ─────────
        _EX_TIPO_PROYECTO = {
            "Mesón de cocina":      "Mesón",
            "Isla de cocina":       "Cocina",
            "Encimera":             "Mesón",
            "Salpicadero / Frente": "Cocina",
            "Baño / Lavamanos":     "Baño",
            "Mueble de baño":       "Baño",
            "Zócalo":               "Otro",
            "Huella escalón":       "Escalera",
            "Escalón completo":     "Escalera",
            "Fachada / Panel":      "Fachada",
            "Personalizado":        "Otro",
        }

        # ── Layout: formulario izq · resultado der ───────────────────────────
        _ex_form_col, _ex_res_col = st.columns([1.1, 1], gap="large")

        with _ex_form_col:
            st.markdown(
                '<div style="font-size:0.67rem;font-weight:800;letter-spacing:0.13em;'
                'color:#1B5FA8;text-transform:uppercase;margin-bottom:2px">Datos del proyecto</div>',
                unsafe_allow_html=True,
            )
            ...
            with st.container(border=True):
                ...
                _refs_ex     = MATERIALES_CATALOGO.get(_ex_cat, []) + ["Otra referencia..."]
                _ex_ref_sel  = st.selectbox("Referencia", _refs_ex, key="ex_ref_sel")
                _ex_ref      = (
                    st.text_input("Nombre de la referencia", placeholder="Ej: Calacatta Oro", key="ex_ref_custom")
                ...
                _ex_pm2 = st.number_input(
                    ...
                    step=5_000, value=st.session_state.get("ex_precio_m2_val", 220_000),
                    key="ex_precio_m2",
                    help="Lo que te cobró el proveedor por m² de lámina.",
                )
                st.session_state["ex_precio_m2_val"] = _ex_pm2

            with st.container(border=True):
                # ── Dimensiones ──────────────────────────────────────────────
                _ex_ancho_lbl = st.selectbox(
                    "📐 Tipo de pieza",
                    list(ANCHOS_ESTANDAR.keys()),
                    key="ex_ancho_tipo",
                )
                _ex_ancho_data = ANCHOS_ESTANDAR[_ex_ancho_lbl]
                _ex_ancho = (
                    st.number_input(
                    ...
                    if _ex_ancho_data["ancho"] is None
                    else _ex_ancho_data["ancho"]
                )
                if _ex_ancho_data["ancho"] is not None:
                    st.caption(f"Ancho: {_ex_ancho} m — {_ex_ancho_data['desc']}")

                _ex_ml = st.number_input(
                    "📏 Metros lineales del proyecto",
                    min_value=0.1, max_value=500.0,
                    step=0.1, value=3.0, format="%.2f",
                    key="ex_ml",
                    help="Suma del largo de todas las piezas del proyecto.",
                )

                # m² calculados en tiempo real
                _ex_m2_calc = round(_ex_ml * _ex_ancho, 3)
                st.markdown(
                    f'<div style="background:rgba(27,95,168,0.07);border-radius:8px;'
                    f'padding:7px 12px;font-size:0.84rem;margin-top:4px">'
                    f'📐 Área del proyecto: <strong>{_ex_ml:.2f} ml × {_ex_ancho:.2f} m'
                    f' = {_ex_m2_calc:.3f} m²</strong></div>',
                    unsafe_allow_html=True,
                )
            ...
            with st.container(border=True):
                ...
                _ex_zona = st.radio(
                    ...
                    ["Local (dentro de la ciudad)", "Regional (20–80 km)", "Foráneo (>80 km)"],
                    horizontal=True,
                    key="ex_zona",
                )
                _ex_km_map = {
                    "Local (dentro de la ciudad)": 5.0,
                    "Regional (20–80 km)":         45.0,
                    "Foráneo (>80 km)":            110.0,
                }
                _ex_km = _ex_km_map[_ex_zona]
                ...
                _ex_iva = st.checkbox("Incluir IVA 19%", value=False, key="ex_iva")
                _ex_cliente = st.text_input(
                    "👤 Cliente", placeholder="Ej: Constructora Ducal (opcional)",
                    key="ex_cliente",
                )

        # ── Cálculo reactivo — sin botón ─────────────────────────────────────
        _ex_props    = PROPIEDADES_MATERIAL.get(_ex_cat, PROPIEDADES_MATERIAL["Mármol"])
        _ex_merma    = _ex_props["merma_base"]
        _ex_area_placa = round(_ex_m2_calc * (1.0 + _ex_merma), 4)
        _ex_tipo_proy  = _EX_TIPO_PROYECTO.get(_ex_ancho_lbl, "Mesón")
        ...
        try:
            _res_ex = calcular_cotizacion_directa(
                ...
                km=_ex_km,
                num_peajes=0,
                foraneo_activo=(_ex_zona != "Local (dentro de la ciudad)"),
                ...
                tipo_proyecto=_ex_tipo_proy,
                nombre_cliente=_ex_cliente or "Sin nombre",
                piezas=[{
                    "nombre":       _ex_tipo_proy,
                    "ml":           _ex_ml,
                    "ml_unitario":  _ex_ml,
                    "cantidad":     1,
                    "ancho":        _ex_ancho,
        ...
        with _ex_res_col:
            st.markdown(
                '<div style="font-size:0.67rem;font-weight:800;letter-spacing:0.13em;'
                'color:#1B5FA8;text-transform:uppercase;margin-bottom:2px">Resultado en tiempo real</div>',
                unsafe_allow_html=True,
            )

            if _ex_err:
                st.error(f"Error en el cálculo: {_ex_err}")
            elif _res_ex:
                # ── Semáforo de margen ────────────────────────────────────────
                _ex_margen_real = _res_ex["margen_pct"]
                if _ex_margen_real >= 30:
                    _sem_color, _sem_icon, _sem_txt = "#1A7A4A", "🟢", "Margen saludable"
                elif _ex_margen_real >= 20:
                    _sem_color, _sem_icon, _sem_txt = "#D35400", "🟡", "Margen aceptable"
                else:
                    _sem_color, _sem_icon, _sem_txt = "#C0392B", "🔴", "Margen bajo — revisa"

                _precio_final_ex = _res_ex["precio_sugerido"] * (1.19 if _ex_iva else 1.0)
                _pml_ex = (_precio_final_ex / _ex_ml) if _ex_ml > 0 else 0

                st.markdown(
                    f'<div style="background:linear-gradient(135deg,#0B2432,#1B5FA8);'
                    f'border-radius:14px;padding:20px 22px;color:white;margin-bottom:12px">'
                    f'<div style="font-size:0.7rem;font-weight:700;opacity:0.55;'
                    f'text-transform:uppercase;letter-spacing:0.1em;margin-bottom:4px">'
                    f'{"Precio con IVA" if _ex_iva else "Precio de venta"}</div>'
                    f'<div style="font-size:clamp(1.6rem,4vw,2.8rem);font-weight:900;'
                    f'font-family:Playfair Display,serif;line-height:1.1;margin-bottom:8px">'
                    f'{cop(_precio_final_ex)}</div>'
                    f'<div style="font-size:0.82rem;opacity:0.75">'
                    f'{cop(_pml_ex)}/ml &nbsp;·&nbsp; '
                    f'Utilidad: {cop(_res_ex["utilidad"])}</div>'
                    f'<div style="margin-top:10px;padding-top:10px;'
                    f'border-top:1px solid rgba(255,255,255,0.15);'
                    f'font-size:0.82rem;font-weight:700;color:{_sem_color}">'
                    f'{_sem_icon} {_sem_txt} — {_ex_margen_real:.1f}%</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

                # ── Desglose de costos ────────────────────────────────────────
                with st.expander("📊 Ver desglose de costos", expanded=True):
                    bloque_costos(
                        [
                            ("Material",   _res_ex["c1_material"]),
                            ("Mano de obra", _res_ex["c2_mano_obra"]),
                            ("Insumos",    _res_ex["c4_insumos"]),
                            ("Logística",  _res_ex["c5_logistica"]),
                        ],
                        "COSTO TOTAL",
                        _res_ex["costo_total"],
                    )
                    st.caption(
                        f"Merma aplicada: {_ex_merma*100:.0f}% ({_ex_cat}) · "
                        f"Área lámina: {_ex_area_placa:.2f} m²"
                    )

                # ── Acciones ──────────────────────────────────────────────────
                st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

                _ex_a1, _ex_a2 = st.columns(2)
                with _ex_a1:
                    if st.button(
                        "💾 Guardar en historial",
                        use_container_width=True,
                        ...
                        if not _ex_cliente.strip():
                            st.warning("Escribe el nombre del cliente para guardar.", icon="⚠️")
                        else:
                            try:
                                import datetime as _dt_ex
                                _ex_num = f"EXP-{_hoy().strftime('%Y%m%d')}-{_ex_cat[:2].upper()}"
                                _guardar_cotizacion(_ex_num, _ex_cliente.strip(), _res_ex)
                                st.success(f"✅ Guardado como **{_ex_num}**")
                            except Exception as _eg:
                                st.error(f"No se pudo guardar: {_eg}")
                with _ex_a2:
                    if st.button(
                        ...
                        _ex_area_completo = _ex_area_placa
    ...
    with t_ia:
        ...
        if not _ia_ok:
            ...
            with _col_vals:
                ...
                # ── Utilidad esperada (AIU u_pct) ────────────────────
                # Lee del store de la cotización AIU activa. Si no hay
                # valor personalizado, usa el default del sistema (5%).
                _aiu_u = float(
                    _sp().get("aiu_u_pct",
                ...
                # ── Gastos Fijos de Taller / m² (CIF) ────────────────
                # Busca la regla "Servicios de Taller" en la receta de
                # Granito (material de referencia; si no existe busca
                # en los demás). Si el wizard aún no se corrió, muestra
                # el fallback del session_state del Paso 3.
                _cif_display: float | None = None
        ...
        if not st.session_state.get("setup_tarifas_completado", False):
            ...
            with _wc:
                ...
                # ════════════════════════════════════════════════════════
                    ...
                    # ════════════════════════════════════════════════════
                    # MODELO HÍBRIDO — 5 bloques de construcción de receta
                    # ════════════════════════════════════════════════════
                    # A. Mano de Obra / Instalación    → base compartida
                    # B. Instalación zócalo            → base compartida
                    # C. Gastos Fijos de Taller (CIF)  → SIEMPRE, valor real Paso 3
                    # D. Consumibles Directos          → CONDICIONAL según política
                    # E. Seguro contra Roturas         → POR MATERIAL (porcentaje dinámico)
                    # ────────────────────────────────────────────────────
                    # NOTA: El Seguro (E) se inyecta DENTRO del loop por material
                    # para poder asignar porcentajes distintos según la fragilidad:
                    #   Sinterizado → 5%  (muy frágil, corte difícil)
                    #   Mármol / Quarztone → 3%  (fragilidad media)
                    #   Granito / resto → 2%  (material resistente)
                    # ────────────────────────────────────────────────────

                    # ── Reglas BASE compartidas (bloques A + B) ───────────
                    # NO incluyen el Seguro contra Roturas — va por separado
                    # en el loop para permitir el porcentaje diferenciado.
                    _receta_base: list = [
                        # A — Mano de Obra / Instalación
                        {
                        ...
                        # B — Zócalo: siempre incluido, se cobra por ML físico de tira
                        {
                            "nombre_interno": "Instalación zócalo",
                            "inductor":       "por_ml_zocalo",
                            "valor":          12_000.0,
                    ...
                    # ── BLOQUE C: Gastos Fijos de Taller (SIEMPRE) ────────
                    # CIF calculado en Paso 3 = recibo_mensual ÷ m²_elaborados_mes.
                    # Fallback $3.000/m² cubre sesiones migradas sin wiz_cif_por_m2.
                    ...
                    # ── BLOQUE D: Consumibles Directos (CONDICIONAL) ──────
                    ...
                    if _politica_insumos == _POL_TALLER_4:
                        ...
                        #   disco ≈$2.200 · lijas ≈$3.800 · pegante ≈$5.000 · masilla ≈$4.000
                        _receta_base += [
                            {
                    ...
                    # else: operarios traen sus insumos → ya absorbido en MO del Paso 2

                    # ── BLOQUE E: Seguro contra Roturas — dinámico por material ──
                    # Se construye el dict final dentro del loop para que cada
                    # material reciba su porcentaje de riesgo apropiado.
                    # La función de porcentaje encapsula las reglas de fragilidad.
                    def _riesgo_rotura(_mat: str) -> float:
                        """Retorna la fracción de riesgo (0–1) según fragilidad del material."""
                        if _mat == "Sinterizado":
                            return 0.05   # 5% — muy frágil, sensible al corte y al golpe
                        if _mat in ("Mármol", "Quarztone"):
                            return 0.03   # 3% — fragilidad media, venas naturales
                        return 0.02       # 2% — Granito y otros: material resistente

                    # ── Inyectar receta en los 5 materiales ──────────────────
                    # Para cada material: clonar la receta base + añadir su
                    # regla de Seguro con el porcentaje correcto al final.
                    _recetas_por_mat: dict = {}
                    for _m in ["Mármol", "Granito", "Sinterizado", "Quarztone", "Quarzita"]:
                        _receta_m = [dict(r) for r in _receta_base]   # clon independiente
                        _receta_m.append({
